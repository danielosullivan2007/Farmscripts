...
p2=plt.plot(felds_int_stats.index, felds_int_stats['20%'],linewidth =0.5, color = 'black')
p2=plt.plot(felds_int_stats.index, felds_int_stats['80%'], linewidth =0.5, color = 'black')
...
green_patch = mpatches.Patch(alpha =0.85 , label='K-Feldspar', lw =1,edgecolor ='k' ,facecolor='gray')
red_circle = mlines.Line2D(range(1), range(1), color="white", marker='o', markerfacecolor="red", label = 'heated')
plt.legend(handles=[blue_patch, green_patch, red_circle], prop={'size':8}, loc=3)
...
#p2=ax1.scatter(indata[:,0], indata[:,1], zorder =20, color ='k', alpha=0.6)
#ax1.scatter(data_key2['T'], data_key2['INPs_perL'], s=14, color='red', zorder=1, label = 'Heated')
bar=pd.read_csv(bardirs['pickels']+'regular_run_INPs.csv')
bon= pd.read_csv(farmdirs['pickels']+'Bonfire.csv')



#ax1.scatter(bar['Temp'], bar['INP'], s=14, color='#641E16', alpha =0.5,
 #           zorder=1, label = 'Barbados \'17')

ax1.scatter(bon['Temp'], bon['INP'], s=14, color='k', alpha =0.5,
            zorder=1, label = 'Leeds City 5th Nov. \'16')
...
plt.yscale('log'), plt.xlim(-30,-5), plt.ylim(0.01, 300)
ax10.set_xlabel('T ('+degree_sign+'C)')
plt.axvline(-20, linestyle ='dashed', color ='k')
plt.legend(fontsize=8)
ax10.set_ylabel('[INPs] ($\mathregular{L^{-1}}$)')

ax11= ax10=plt.subplot(122)
plt.yscale('log'), plt.xlim(-30,-5), plt.ylim(0.01, 300)
ax10.set_xlabel('T ('+degree_sign+'C)')
